...
class ParamedicRunner {
    ...
    run () {
        ...
        return Q().then(() => {
            ...
            return Q();//aramedicApp.prepareProjectToRunTests();
        })
            .then(() => {
    ...
    runAppiumTests (useSauce) {
        ...
        let options = {
            ...
            pluginRepos: this.config.getPlugins() ? this.config.getPlugins().map(plugin => path.join(this.tempFolder.name, 'plugins', path.basename(plugin))) : null,
