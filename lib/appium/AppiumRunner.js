...
class AppiumRunner {
    ...
    findTests () {
        ...
        searchPaths.push(path.join(this.options.appPath, 'tests', 'appium-tests', this.options.platform));
        searchPaths.push(path.join(this.options.appPath, 'tests', 'appium-tests', 'common'));
...
function getPluginDirs (appPath) {
    return shell.ls('-d', path.join(appPath, 'plugins', 'elastos-trinity-plugins-*'));
